...
router.get('/', async (req, res) => {
    try {
        const { status, sector, limit, offset, sort } = req.query;
        ...
        const result = await company_repository_1.companyRepository.list({
            status: status,
            sector,
...
router.get('/:ticker', async (req, res) => {
    try {
        ...
        const company = await company_repository_1.companyRepository.findByTickerWithMarketData(ticker);
        if (!company) {
...
router.get('/:ticker/analysis', async (req, res) => {
    try {
        ...
        const analysis = await analysis_repository_1.analysisRepository.findCurrentByCompanyId(company.id);
        if (!analysis) {
            return res.status(404).json({
                error: 'Not Found',
 ...
 */
router.get('/:ticker/analyses', async (req, res) => {
    try {
        const { ticker } = req.params;
        ...
        // Find analyses
        const analyses = await analysis_repository_1.analysisRepository.findByCompanyId(company.id, {
            status: status,
