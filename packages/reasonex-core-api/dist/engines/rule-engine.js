...
class RuleEngine {
    ...
    constructor(logger, configPath) {
        this.logger = logger || new logger_1.Logger(undefined, { service: 'reasonex-core-api', node: 'RuleEngine' });
        this.tracer = new tracer_1.Tracer('RuleEngine', this.logger);
    ...
    loadRuleSets() {
        try {
            ...
            this.logger.error('Failed to load rule sets', { error: error.message });
        }
    }
    /**
     ...
     */
    getValue(data, field, defaultValue = 0) {
     ...
     * Total: 100 points
     */
    scoreInvestment(data, debugMode = false) {
     ...
     */
    score(data, ruleSetId, context, debugMode = false) {
     ...
     */
    scoreGeneric(data, ruleSetId, context, debugMode = false) {
        return this.tracer.withSpanSync('scoreGeneric', (span) => {
            ...
            const ruleSet = this.ruleSets.get(ruleSetId);
            if (!ruleSet) {
                span.setStatus(tracer_1.SpanStatus.ERROR, `Rule set not found: ${ruleSetId}`);
     ...
     */
    evaluateOperator(value, operator, target) {
        switch (operator) {
            case 'lt':
                return value < target;
            case 'lte':
     ...
     */
    calculateScore(value, formula, maxScore, minScore = 0) {
        try {
            ...
            this.logger.warn('Score formula evaluation failed', { formula, value, error: error.message });
            return 0;
        }
    }
    /**
     ...
     */
    classify(score, classifications) {
...
exports.default = exports.ruleEngine;
